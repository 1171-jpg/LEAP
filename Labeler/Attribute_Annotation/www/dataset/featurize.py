...
def get_tensor_dataset(dataset, label_key='plausible', add_spans=False, add_segment_ids=True, masked_lm_params=None):
...
# Creates tensor dataset from featurized dataset
def get_tensor_dataset_tiered(dataset, max_sentences, add_segment_ids=True):
...
def add_bert_features_ConvEnt(dataset, tokenizer, seq_length, add_segment_ids=True):
...
def add_bert_features_art(dataset, tokenizer, seq_length, add_segment_ids=True):
  for i, ex in enumerate(dataset):
...
def add_bert_features_tiered(dataset, tokenizer, seq_length, add_segment_ids=True):
  if add_segment_ids:
    print("Yes")
