...
def add_abstract(tiered_dataset,abstract_dict):
    for key in tiered_dataset.keys():
        for data in tqdm(tiered_dataset[key]):
            for sample in data['stories']:
                ...
                for ent in sample['entities']:
                    ent['abs_sentences']=abstract_sentences
                    if ent['entity'] in abstract_dict.keys() and abstract_dict[ent['entity']]['parent']!='entity.n.01':
                        ent['abs_class']= abstract_dict[ent['entity']]['parent'].split(".")[0]
                    else:
                        ent['abs_class']= ent['entity']
...
def get_tiered_data_cut(dataset):
    # states = {p: [] for p in dataset}
    max_story_length = max(
        [len(ex['sentences']) for ex_2s in dataset for ex in ex_2s['stories']])
    print(max_story_length)
    for ex_2s in dataset:
        for s_idx, ex in enumerate(ex_2s['stories']):
            if 'states' in ex:
                ent_sent_examples = {}
                all_entities = set()
                for i, sent_ann in enumerate(ex['states']):
                    ...
                    for att in sent_ann:
                        for ent, v in [
                                tuple(ann) for ann in sent_ann[att]
                        ]:
                            entities.append(ent)
                            if ent not in entity_anns:
                                entity_anns[ent] = [
                                    [0] * len(att_to_idx),
                                    [0] * len(att_to_idx)
                                ]  # pre/post condition, then value for each attribute
                            if 'location' not in att:
                                entity_anns[ent][0][
                                    att_to_idx[att]] = att_change_dir[
                                        'default'][v][0] + 1
                                entity_anns[ent][1][
                                    att_to_idx[att]] = att_change_dir[
                                        'default'][v][1] + 1
                    ...
                    for ent in entities:
                        states_ex = {}
                        states_ex['example_id'] = ex_2s[
                            'example_id'] + '-%s-%s-%s' % (str(s_idx),
                                                           str(i), ent)
                        states_ex['base_id'] = ex['example_id']
                        states_ex['sentence_idx'] = i
                        states_ex['entity'] = ent
            ...
            ex_2s['stories'][s_idx]['entities'] = [
                None for _ in range(len(all_entities))
            ]  # entity-story data: preconditions, effects, etc.
            for ei, ent in enumerate(all_entities):
                ent_ex = {}
                ent_ex['example_id'] = ex_2s['example_id'] + '-' + str(
                    s_idx) + '-' + ent
                ent_ex['base_id'] = ex_2s['example_id'] + '-' + str(s_idx)
                ent_ex['sentences'] = ex['sentences']
                ent_ex['entity'] = ent
                ent_ex['attributes'] = np.zeros(
                    (max_story_length, len(att_to_idx)))
                ent_ex['preconditions'] = np.zeros(
                    (max_story_length, len(att_to_idx)))
                ent_ex['effects'] = np.zeros(
                    (max_story_length, len(att_to_idx)))

                ent_ex['conflict_span'] = (0, 0)
                ent_ex['conflict_span_onehot'] = np.zeros(
                    (max_story_length))
                ent_ex['plausible'] = 1
                if s_idx != ex_2s['label'] and ex_2s['label'] != -1:
                    # print(ex_2s['confl_sents'])
                    conflict_span = (max([
                        s + 1 for s in ex_2s['confl_sents']
                        if s < ex_2s['breakpoint']
                    ]), ex_2s['breakpoint'] + 1)

                    # Check if the entity has some nontrivial annotated states in the boundaries of the conflict span
                    if (ent, conflict_span[0] - 1) in ent_sent_examples:
                        for i, att in enumerate(att_default_values):
                            if (ent_sent_examples[(ent, conflict_span[0] -
                                                   1)]['preconditions'][i]
                                    != att_default_values[att]
                                    or ent_sent_examples[
                                        (ent, conflict_span[0] -
                                         1)]['effects'][i] !=
                                    att_default_values[att]):
                                ent_ex['conflict_span'] = conflict_span
                    ...
                    if (ent, conflict_span[1] - 1) in ent_sent_examples:
                        for i, att in enumerate(att_default_values):
                            if (ent_sent_examples[(ent, conflict_span[1] -
                                                   1)]['preconditions'][i]
                                    != att_default_values[att]
                                    or ent_sent_examples[
                                        (ent, conflict_span[1] -
                ...
                # Get binary label for each span of text as well (for alternative formulation)
                ent_ex['span_labels'] = np.zeros(
                    (max_story_length * (max_story_length - 1) // 2))
                if s_idx == 1 - ex_2s[
                        'label']:  # If this is the implausible choice
                    span_idx = 0
                    for s2 in range(1, len(ex['sentences'])):
                        for s1 in range(s2):
                            # print(ex['confl_pairs'])
                                ...
                                if s1 <= p1 and s2 >= p2:
                                    ...
                                    if (ent, p1) in ent_sent_examples:
                                        for i, att in enumerate(
                                                att_default_values):
                                            if ent_sent_examples[(
                                                    ent, p1
                                            )]['preconditions'][
                                                    i] != att_default_values[
                                                        att] or ent_sent_examples[(
                                                            ent, p1
                                                        )]['effects'][
                                                            i] != att_default_values[
                                                                att]:
                ...
                for i in range(ex_2s['length']):
                    if (ent, i) in ent_sent_examples:
                        ent_ex['preconditions'][i, :] = ent_sent_examples[(
                            ent, i)]['preconditions']
                        ent_ex['effects'][i, :] = ent_sent_examples[(
                            ent, i)]['effects']
                        for j, att in enumerate(att_default_values):
                            if ent_ex['preconditions'][
                                    i, j] != att_default_values[
                                        att] or ent_ex['effects'][
                                            i,
                                            j] != att_default_values[att]:
                                ent_ex['attributes'][i, j] = 1
                ex_2s['stories'][s_idx]['entities'][ei] = ent_ex
