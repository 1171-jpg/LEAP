...
class TieredModelPipeline(nn.Module):
    def __init__(
        ...
        self.device = device
        drop_out = getattr(embedding_config, "cls_dropout", None)
        if drop_out is None:
            drop_out = getattr(embedding_config, "dropout_rate", None)
        if drop_out is None:
            drop_out = getattr(embedding_config, "hidden_dropout_prob", None)
        assert drop_out is not None, "Didn't set dropout!"
        self.dropout = nn.Dropout(drop_out)
        ...
        self.attribute_classifier = nn.Linear(embedding_config.hidden_size,
                                              num_attributes)
        ...
        for i in range(num_attributes):
            ...
            self.precondition_classifiers.append(
                ClassificationHead(config, input_all_tokens=False).to(device))
            self.effect_classifiers.append(
                ClassificationHead(config, input_all_tokens=False).to(device))
        self.precondition_classifiers = nn.ModuleList(
            self.precondition_classifiers)
        ...
        if 'embeddings' not in ablation:
            self.embedding_proj = nn.Linear(embedding_config.hidden_size,
                                            embedding_proj_size)
        if 'states' not in ablation:
            self.states_proj = nn.Linear(self.states_size, embedding_proj_size)
        if 'states-attention' in ablation:
            self.states_attention = nn.Sequential(
                nn.Linear(self.states_size, embedding_proj_size),
                nn.Sigmoid())  # Attention mechanism conditioned on states
        ...
        assert self.encoding_size % n_head == 0, "Conflict detector encoding size (%s) should be divisible by n_heads=%s" % (
            str(self.encoding_size), str(n_heads))
        transformer_layers = nn.TransformerEncoderLayer(d_model=encoding_size,
                                                        nhead=n_head,
                                                        dim_feedforward=4 *
                                                        encoding_size,
                                                        dropout=drop_out,
        ...
        self.decoder = nn.Linear(
            num_sents * encoding_size,
            num_sents)  # pivot point multilabel classification layer
...
#         batch_size, num_stories, num_entities, num_sents, seq_length = input_ids_shape
...
#                                        num_sents)
        ...
        # 1) Embed the inputs
        out = self.tslm(input_ids=input_ids,attention_mask=attention_mask
             ,timestep_type_ids=timestep_type_ids)

        #     if len(out[0].shape) < 3:
        #       out[0] = out[0].unsqueeze(0)
        out = out[:, 0, :]  # entity-sentence embeddings
        ...
        out_preconditions = torch.zeros(
            (self.num_sents,
             self.num_attributes)).to(self.device)
        out_preconditions_softmax = torch.tensor([]).to(self.device)
        for i in range(self.num_attributes):
            out_s = self.precondition_classifiers[i](out,
                                                     return_embeddings=False)

            with torch.no_grad(
            ):  # Don't allow backprop from conflict detection to state classifiers
                out_preconditions_softmax = torch.cat(
                    (out_preconditions_softmax, out_s), dim=-1)


            out_preconditions[:, i] = torch.argmax(
                out_s, dim=1)  # Extract predicted value

            if preconditions is not None:
                loss_preconditions += loss_fct(
                    out_s.view(-1,
                               self.precondition_classifiers[i].num_labels),
                    preconditions[ :, i].view(-1))

#         out_preconditions *= length_mask.view(-1).repeat(
#             self.num_attributes,
#             1).t()  # Mask out any nonexistent entities or sentences
#         assert length_mask.view(-1).shape[0] == out_preconditions.shape[0]
        return_dict[
            'out_preconditions'] = out_preconditions  # * length_mask.view(-1).repeat(self.num_attributes, 1).t()
        if preconditions is not None:
        ...
        for i in range(self.num_attributes):
            ...
            with torch.no_grad(
                ...
                out_effects_softmax = torch.cat((out_effects_softmax, out_s),
                                                dim=-1)


            out_effects[:, i] = torch.argmax(out_s,
                                             dim=1)  # Extract predicted value

            if effects is not None:
                loss_effects += loss_fct(
                    out_s.view(-1, self.effect_classifiers[i].num_labels),
                    effects[ :, i].view(-1))

#         out_effects *= length_mask.view(-1).repeat(
#             self.num_attributes,
#             1).t()  # Mask out any nonexistent entities or sentences
#         assert length_mask.view(-1).shape[0] == out_effects.shape[0]
        return_dict[
            'out_effects'] = out_effects  # * length_mask.view(-1).repeat(self.num_attributes, 1).t()
        if effects is not None:
        ...
        out = torch.cat(
            (out, torch.zeros(out.shape[:-1] +
                              (self.encoding_pad_zeros, )).to(self.device)),
            dim=-1)
        out = out.view(self.num_sents, -1)
...
#         out = self.detector(out.transpose(0, 1)).transpose(0, 1)
        ...
        out = torch.sigmoid(
            out
        ) * sentence_mask  # Do sigmoid again since this happened inside loss function
...
#         out = out.view(
#             batch_size, num_stories, num_entities,
#             -1)  # Reshape to one prediction per example-story-entity triples
#         out = -torch.sum(
#             out, dim=(2, 3)
#         ) / 2  # divide by 2 so the expected sum is 1 for conflicting story (2 conflicting sentences)
...
#             return_dict['loss_stories'] = loss_stories
        ...
        if loss_conflicts is not None:
            total_loss +=  loss_conflicts

        if total_loss is not None:
            return_dict['total_loss'] = total_loss
#         if loss_preconditions is not None:
#             total_loss += self.loss_weights[
#                 1] * loss_preconditions / self.num_attributes
#         if loss_effects is not None:
#             total_loss += self.loss_weights[
#                 2] * loss_effects / self.num_attributes
#         if loss_conflicts is not None:
