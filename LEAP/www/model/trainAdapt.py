...
import progressbar
from www.model.evalAdapt import evaluate_tiered
...
def train_epoch_tiered(MaxStoryLength,tslm_model,trip_model,tslm_optimizer,trip_optimizer,train_dataloader, device, seg_mode=False, return_losses=False, build_learning_curves=False, val_dataloader=None, train_lc_data=None, val_lc_data=None):
  t0 = time.time()
  ...
  for step, batch in enumerate(train_dataloader):
    ...
    output=tslm_model(input_ids=input_ids.view(-1,MaxStoryLength),attention_mask=input_mask.view(-1,MaxStoryLength)
             ,timestep_type_ids=timestep_type_ids.view(-1,MaxStoryLength))
    out=trip_model(output,
          input_ids.shape,
          input_lengths,
          input_entities,
          attention_mask=input_mask,
    ...
    if build_learning_curves:
      ...
      if (len(train_dataloader) - step - 1) % chunk_size == 0:
        validation_results = evaluate_tiered(MaxStoryLength,tslm_model,trip_model, val_dataloader, device, [(accuracy_score, 'accuracy'), (f1_score, 'f1')], seg_mode=False, return_explanations=True, return_losses=True, verbose=False)
