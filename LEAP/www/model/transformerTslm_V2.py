...
class TieredModelPipeline(nn.Module):
  def __init__(self, embedding_config,num_sents, num_attributes, labels_per_att, config_class, model_name, device, ablation=[]): # labels_per_att is a dictionary mapping attribute index to number of labels
    super().__init__()
    ...
    self.attribute_classifier = nn.Linear(embedding_config.hidden_size, num_attributes)
    ...
    if 'embeddings' not in ablation:
      self.embedding_proj = nn.Linear(embedding_config.hidden_size, embedding_proj_size)
    if 'states' not in ablation:
...
#     self.loss_weights = loss_weights
  ...
  def forward(self,input_cls,input_ids_shape, input_lengths, input_entities,out_preconditions,out_preconditions_softmax,out_effects,out_effects_softmax, attention_mask=None, token_type_ids=None, attributes=None, preconditions=None, effects=None, conflicts=None, labels=None, training=False):
    ...
    # 1) Embed the inputs
    out=input_cls # entity-sentence embeddings
